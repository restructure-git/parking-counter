...
MVP（実用最小限の製品）** です。クラウドやLLM/AI APIを使わず、Windows PC上で
ローカルに完結する画像処理だけで「空き／使用中／判定不能」を判定します。
判定方式は2種類から選べます（詳細は第10章）。

- **画像差分方式（既定）**: OpenCVの単純な画素差分。追加インストール不要。
- **YOLO方式（任意）**: ローカルで動く軽量な物体検出モデルで車両を検出。
  画像差分方式より頑丈だが、追加のインストール（数百MB〜1GB）が必要。

いずれもLLM（文章生成AI）やcrowd API は使わず、すべてPC上で完結します。
...
Windows PC上のPythonサーバー（FastAPI + Uvicorn）
    ↓ 駐車枠ごとに空き・使用中を判定（画像差分 or ローカルYOLO）
    ↓ 3回連続で同じ判定が出たら状態を確定
...
## 9. 基準画像の登録方法（画像差分方式のみ）

判定方式を既定の画像差分方式のまま使う場合に必要な手順です。YOLO方式を使う
場合はこの章は不要です（第10章）。
...
- `occupied_threshold`: 画像差分方式で、この差分率を超えたら「使用中」寄りと判定します。
...
### 10.1 YOLO方式への切り替え（任意）

画像差分方式は、駐車枠が単色でベタ塗りされたような特殊なケースだと明るさの
正規化で差が打ち消されて検知できなかったり、夜間・逆光に弱かったりします。
これをローカルで動く軽量な物体検出モデル（YOLO）に差し替えることで、
車両の形そのものを認識して判定できるようになり、より頑丈になります。
クラウドやLLMは使わず、CPU上でローカルに完結します（GPU不要）。

1. 追加の依存パッケージをインストールします（torchを含むため数百MB〜1GB
   程度のダウンロードが発生します）。
   ```bat
   .venv\Scripts\python.exe -m pip install -r requirements-yolo.txt
   ```
2. `config/settings.json` に以下を追加します。
   ```json
   {
   ...
   ```
3. サーバーを再起動します。初回の判定実行時に、モデルファイル
   （`yolov8n.pt`、約6MB）が自動的にダウンロードされます
   （要インターネット接続、2回目以降はキャッシュを使います）。

設定項目の意味:

- `yolo_model_path`: 使用するモデルファイル（既定は最軽量のnanoモデル）。
- `yolo_confidence_threshold`: この confidence 未満の検出は無視します。
- `yolo_overlap_threshold`: 駐車枠の面積に対して、検出した車両とどれだけ
  重なったら「使用中」寄りと判定するか。
- `yolo_uncertain_margin`: しきい値の前後どのくらいを「判定不能」の幅とするか。

YOLO方式では基準画像の登録（第9章）は不要です。駐車枠を登録すればすぐに
判定できます。また `ultralytics` が未インストール、またはモデルの初期化に
失敗した場合は、エラーで停止せず自動的に画像差分方式にフォールバックします
（起動ログに warning が出ます）。
...
YOLO方式のテスト（`tests/test_yolo_detector.py`）は、幾何計算のみを対象と
しており `ultralytics` のインストールを必要としません。
...
- 駐車枠の座標は、判定時に画像をリサイズする幅（`image_max_width`）を
  ...
  ずれる場合があります。
- 既定の画像差分方式（グレースケール＋GaussianBlur＋明るさ正規化＋
  絶対差分）は、夜間や強い逆光、積雪などの条件では精度が下がる可能性が
  あります（YOLO方式でも夜間・悪天候での精度低下は起こり得ます）。
...
- 基準画像として保存されるのは、駐車枠部分を切り出した小さな画像のみです
  （画像差分方式のみ。YOLO方式では基準画像自体を保存しません）。
- SQLiteに保存されるのは、判定時刻・空き台数・駐車枠ごとの状態と差分率
  （またはYOLOの重なり率）のみで、画像データやナンバープレート情報は
  一切保存しません。
...
- 判定処理そのもの（画像差分・YOLOのどちらも）はPC上でローカルに実行され、
  クラウドAPIやLLMには一切送信されません。
...
- YOLOモデルを自前データでファインチューニングし、精度をさらに上げる
...
- クラウドサービス・LLM/外部AI APIを使用しない（判定ロジック（画像差分・
  YOLOのどちらも）はローカルのみで完結。Cloudflare Tunnelは公開経路であり、
...
│   ├── services/
│   │   ├── parking_detector.py    画像差分方式の判定ロジック
│   │   ├── yolo_detector.py       YOLO方式の判定ロジック（任意）
│   │   └── ...                    状態管理・画像/基準画像管理
│   ├── routers/                   dashboard / detection / history / admin
│   ├── templates/                 Jinja2テンプレート（HTML）
│   └── static/                    CSS / JS
├── config/settings.json           しきい値・判定方式などの設定
├── data/                          駐車枠定義・基準画像・SQLite DB（元画像は含まない）
├── tests/                         pytest（画像差分・YOLO幾何計算・状態管理・API）
├── scripts/
│   ├── create_sample_data.py      テスト用の合成画像生成
│   └── check_credentials.py       起動前の認証情報チェック
├── requirements.txt                既定（画像差分方式）の依存パッケージ
├── requirements-yolo.txt           YOLO方式を使う場合に追加インストール
├── run.bat                         同じWi-Fi内で起動
